...
const StudentCertificates: React.FC<Props> = ({ filter }) => {
    ...
    const filteredCertificates: LFCertificate[] = useMemo(() => {
        ...
        return data?.me?.student?.participationCertificates.filter((cert: LFCertificate) => cert.state === filter);
...
const CertificateRow: React.FC<CertificateProps> = ({ certificate }) => {
    ...
    return (
        <Card flexibleWidth>
            <Container padding={space['1']} width="100%" maxWidth="100%" alignItems="stretch">
                <Row space={space['1']} marginBottom={space['1']} alignItems="center">
                    <Column>{/* <ProfilAvatar size="md" image={avatar} /> */}</Column>
                    <Column></Column>
                </Row>
                <Row flexDirection="column" alignItems="stretch" space={space['0.5']} marginBottom={space['1']}>
                    ...
                    {certificate.startDate && (
                        <Column flexDirection="row">
                            ...
                            <Text>{DateTime.fromISO(certificate.startDate).toFormat('dd.MM.yyyy')}</Text>
                        </Column>
                    )}
                </Row>
            </Container>
