...
const IndicatorBar: React.FC<IndicatorBarProps> = ({
    ...
    const flexDirection = useBreakpointValue({
        base: achievementType === Achievement_Type_Enum.Streak ? 'column' : isCard ? 'row-reverse' : 'column-reverse',
    ...
    const textWidth = useBreakpointValue({
        base: achievementType === Achievement_Type_Enum.Streak || !isCard ? '100%' : '20%',
    ...
    const finishedStepsInformation = useBreakpointValue({
        base: isCard ? `${currentStep}/${maxSteps}` : `${t('achievement.card.finishedStepsInformation', { currentStep, maxSteps })}`,
