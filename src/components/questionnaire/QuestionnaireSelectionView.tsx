...
import { Answer, ObjectAnswer, QuestionnaireContext, SelectionQuestion } from '../Questionnaire';
...
const QuestionnaireSelectionView: React.FC<Props> = ({ currentQuestion, userType, prefill, onPressSelection }) => {
    ...
    const Wrapper = (currentQuestion.viewType || 'normal') === 'normal' ? TwoColGrid : Column;

    return (
        <VStack paddingX={space['1']} paddingTop={space['1']}>
            <Heading>{t(`registration.questions.${userType}.${currentQuestion.id}.question`)}</Heading>
                    ...
                    (opt, index) =>
                        (currentQuestion.viewType === 'large' && (
                            ...
                            <IconTagList
                                ...
                                text={opt.label}
                                variant="selection"
                                textIcon={(currentQuestion.imgRootPath === 'text' && opt.key) || undefined}
                                iconPath={(currentQuestion.imgRootPath !== 'text' && `${currentQuestion.imgRootPath}/icon_${opt.key}.svg`) || undefined}
                                onPress={() => {
