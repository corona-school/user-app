...
export const LFApolloProvider: React.FC<{ children: ReactNode }> = ({
  ...
  return <ExtendedApolloContext.Provider value={context}>
...
class RetryOnUnauthorizedLink extends ApolloLink {
      ...
      nextChain.subscribe(it => {
        if (it.errors?.length) {
          ...
          if (isAuthError && !!getDeviceToken()) {
            ...
            this.loginWithDeviceToken(forward!, observer)
              .then(succeeded => {
                if (succeeded) {
  ...
  async loginWithDeviceToken(forward: NextLink, observer: SubscriptionObserver<SomeFetchResult>): Promise<boolean> {
    const login = forward!(createOperation({}, {
      query: gql`
    ...
    const firstResult = await new Promise<SomeFetchResult>(res => login.subscribe(res));
...
const useApolloInternal = () => {
  ...
  const client = useMemo(() => {
    ...
    const uriLink = new HttpLink({
      ...
      return new ApolloClient({
        ...
        link: from([new RetryOnUnauthorizedLink(onMissingAuth), tokenLink, uriLink])
      });
    },
    [onMissingAuth]
  );
  ...
  useEffect(() => {
    (async function() {
      ...
      if (legacyToken) {
        if (getDeviceToken()) {
          console.log(`Legacy Token present in URL, but as a device token is already present it is not used`);
          return;
        }
        ...
        if (result.errors?.length) {
          console.warn(`Failed to log in with a legacy token, token is probably invalid`);
      ...
      if (secretToken) {
        if (getDeviceToken()) {
          console.log(`Secret Token present in URL, but as a device token is already present it is not used`);
          return;
        }
        ...
        if (result.errors?.length) {
          console.warn(`Failed to log in with a secret token, token is probably invalid`);
          return; // Silently failing, maybe the user knows his password instead?
        }
  ...
  return useMemo(() => ({ client, logout, createDeviceToken, sessionState }), [client, logout, createDeviceToken, sessionState]);
}
