...
const RequestMatchPreview: React.FC<Props> = ({ subjects, classes, onRequestMatch, onBack, disableButton, disableReason }) => {
    ...
    return (
        <VStack marginX="auto" space={space['1']} maxWidth={ContainerWidth}>
            ...
            {subjects.map((sub: LFSubject, index: number) => {
                return (
                    <VStack paddingBottom={space['1']}>
                        ...
                        <IconTagList variant="center" text={sub.name} isDisabled iconPath={`subjects/icon_${sub?.name?.toLowerCase()}.svg`} />
                        ...
                        <Box maxWidth="fit-content">
                            <ToggleButton
                                dataKey={`${index}`}
                                isActive={false}
                                label={`${classes[sub.name] && classes[sub.name].min}. - ${classes[sub.name] && classes[sub.name].max}. Klasse`}
                            />
                        </Box>
                    </VStack>
                );
            ...
            <Row space={space['1']} flexDirection={ButtonContainerDirection}>
                <Button marginBottom={space['1']} width={ButtonContainer} onPress={onRequestMatch} isDisabled={disableButton}>
