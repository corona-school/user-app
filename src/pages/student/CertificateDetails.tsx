...
const CertificateList: React.FC = () => {
    ...
    const PupilStatus: React.FC<{
        ...
        const textColor = useMemo(() => (variant === 'dark' ? 'lightText' : 'darkText'), [variant]);
        return (
            <Card flexibleWidth padding={space['1']} variant={variant}>
                ...
                <CollapsibleContent isOpen={isOpen} textColor={textColor} onPressHeader={() => setIsOpen((prev) => !prev)}>
                    <VStack mt={space['0.5']} space={'1'}>
                        <Text color={textColor}>Fach: {pupil.subjectsFormatted?.join(',')}</Text>
                        <Text color={textColor}>Startdatum: {DateTime.fromISO(pupil.startDate).toFormat('dd.MM.yyyy')}</Text>
                        <Text color={textColor}>Enddatum: {DateTime.fromISO(pupil.endDate).toFormat('dd.MM.yyyy')}</Text>
                        <Text color={textColor}>Zeitl. Aufwand: {pupil.hoursPerWeek} Stunden</Text>
        ...
        >
            ...
            {(certificate.uuid || true) && (
                <VStack space={space['1']} paddingX={space['1']}>
                    ...
                    <Button onPress={updateCertificates}>Bescheinigungen aktualisieren</Button>
                    ...
                    {certificateType === 'group' && (
                        <VStack space={space['1']}>
                            <AppointmentCard
                                ...
                                dateNextLecture={DateTime.now().toISO()}
                                title={'Kursname'}
