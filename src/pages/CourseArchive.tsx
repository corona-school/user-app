...
const studentQuery = gql`
    query GetSubcoursesInstructingArchive {
        me {
            student {
...
const pupilQuery = gql`
    query GetSubcoursesJoinedArchive {
        me {
            pupil {
                subcoursesJoined {
                    id
...
const CourseArchive: React.FC<Props> = () => {
    ...
    return (
        <WithNavigation headerTitle={t('archive.course.header')} headerLeft={<NotificationAlert />}>
            <VStack paddingX={space['1']} marginX="auto" marginBottom={space['2.5']} width="100%" maxWidth={ContainerWidth}>
                <VStack space={space['1']}>
                    <VStack space={space['0.5']}>
                        <Heading>{t('archive.course.title')}</Heading>
                        <Text maxWidth={ContentContainerWidth}>{t('archive.course.content')}</Text>
                    </VStack>
                    ...
                    <VStack space={space['1']}>
                        ...
                        <Text maxWidth={ContentContainerWidth}>{t('archive.course.sectionContent')}</Text>
                    </VStack>
                    <VStack flex="1">
                        {loading && (
                            <Box mt="5">
                                    ...
                                    <CSSWrapper className="course-list__wrapper">
                                        {searchResults.map((sub: LFSubCourse, index: number) => {
                                            ...
                                            for (const lecture of sub.lectures) {
                                                ...
                                                if (date.diff(firstDate).toMillis() < firstDate.toMillis()) {
                                                    firstDate = date;
                                                }
                                            }
                                                    ...
                                                    <AppointmentCard
                                                        ...
                                                        onPressToCourse={() => {
                                                            trackEvent({
                                                                category: 'kurse',
                                                                action: 'click-event',
                                                                name: 'Kurs Archive – Kachel: ' + sub.course.name,
