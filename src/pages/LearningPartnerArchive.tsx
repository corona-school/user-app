import { Text, Heading, useTheme, VStack, Input, useBreakpointValue, Column, Row, Button, Spinner, Box } from 'native-base';
...
const LearningPartnerArchive: React.FC<Props> = () => {
    ...
    const activeMatches = useMemo(() => data?.me?.pupil?.matches.map((match: LFMatch) => !match.dissolved), [data?.me?.pupil?.matches]);
        ...
        return (
            activeMatches?.filter((match: LFMatch) =>
                match.student.firstname
                    .toLowerCase()
                    .includes(searchString.toLowerCase() || match.student.lastname.toLowerCase().includes(searchString.toLowerCase()))
            ) || []
        );
    ...
    return (
        <WithNavigation headerTitle={t('archive.learningpartner.header')} headerLeft={<NotificationAlert />}>
            <VStack paddingX={space['1']} marginX="auto" maxWidth={ContainerWidth}>
                <VStack space={space['1']}>
                    <VStack space={space['0.5']}>
                        <Heading>{t('archive.learningpartner.title')}</Heading>
                        <Text maxWidth={ContentContainerWidth}>{t('archive.learningpartner.content')}</Text>
                    </VStack>
                    ...
                    <VStack space={space['1']}>
                        ...
                        <Text maxWidth={ContentContainerWidth}>{t('archive.learningpartner.sectionContent')}</Text>
                    </VStack>
                    <VStack flex="1">
                        {loading && (
                            <Box mt="5">
                                                            ...
                                                            <Button
                                                                onPress={() => {
                                                                    trackEvent({
                                                                        ...
                                                                        documentTitle: 'Lernpartner Matching Archive',
                                                                    });
                                                                }}
