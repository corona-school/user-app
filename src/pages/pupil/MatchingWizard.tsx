...
import { Text, VStack, Heading, Button, useTheme, Modal, useBreakpointValue, Row } from 'native-base';
...
const MatchingWizard: React.FC<Props> = () => {
    ...
    const [createMatchRequest, { data: requestData, error: requestError, loading: requestLoading }] = useMutation(gql`
    ...
    return (
        <>
            <VStack space={space['1']} paddingX={space['1']} width="100%" marginX="auto" maxWidth={ContainerWidth}>
                <Heading>{t('matching.request.headline')}</Heading>
                <Text maxWidth={ContentContainerWidth}>{t('matching.request.content')}</Text>
                <Heading fontSize="lg">{t('matching.request.yourDetails')}</Heading>

                <VStack space={space['0.5']} maxWidth={ContentContainerWidth}>
                    <Text>
                        <Text bold>{t('matching.request.schoolType')}</Text> <Text>{getSchoolTypeKey(SelectedSchoolType)}</Text>
                    </Text>
                    <Text>
                        <Text bold>{t('matching.request.grade')}</Text> {data?.me?.pupil?.gradeAsInt}
                    </Text>
                </VStack>
                <VStack space={space['0.5']} maxWidth={ContentContainerWidth}>
                ...
                <Row flexDirection={MatchingButton} space={space['1']} marginBottom={space['1.5']}>
                    <Button
                    ...
                    {!data?.me?.pupil?.canRequestMatch?.allowed && <Text>{t(`lernfair.reason.${data?.me?.pupil?.canRequestMatch?.reason}.matching`)}</Text>}
                    <Button
                        variant={'outline'}
                        onPress={() => {
                            trackEvent({
            ...
            >
                <Modal.Content>
                    <Modal.CloseButton />
                    <Modal.Header>{t('matching.request.modalSuccess.title')}</Modal.Header>
