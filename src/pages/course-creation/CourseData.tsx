...
const CourseData: React.FC<Props> = ({ onNext, onCancel, onShowUnsplash, onShowAddInstructor, onRemoveInstructor }) => {
            ...
            <VStack space={space['1']} marginX="auto" width="100%" maxWidth={ContentContainerWidth}>
                ...
                <FormControl marginBottom={space['0.5']}>
                    ...
                    <Input
                        ...
                        onChangeText={(text) => setCourseName && setCourseName(text.substring(0, MAX_TITLE))}
                    />
                    ...
                    <Box>
                        <Slider
                            ...
                            onValueChange={(value: number | number[]) => {
                                Array.isArray(value) && setClassRange && setClassRange([value[0], value[1]]);
                            }}
                        />
                    </Box>
                </FormControl>
                ...
                <FormControl marginBottom={space['0.5']}>
                    ...
                    <Box paddingY={space['1']}>
                        <WidgetUnsplash photo={pickedPhoto} prefill={image} onShowUnsplash={onShowUnsplash} onDeletePhoto={deletePhoto} />
                    </Box>
                </FormControl>
                <FormControl marginBottom={space['0.5']}>
                    ...
                    <VStack mt={space['1']}>
                        {addedInstructors &&
                            addedInstructors.map((instructor: LFInstructor, index: number) => (
                                <InstructorRow instructor={instructor} onPressDelete={() => onRemoveInstructor && onRemoveInstructor(index, true)} />
                            ))}
                        {newInstructors &&
                            newInstructors.map((instructor: LFInstructor, index: number) => (
                                <InstructorRow instructor={instructor} onPressDelete={() => onRemoveInstructor && onRemoveInstructor(index, false)} />
                            ))}
                    </VStack>
