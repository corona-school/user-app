...
import { Button, Text, Heading, useTheme, VStack, Row, Column, Input, FormControl, Stack, Modal, useBreakpointValue, Box } from 'native-base';
...
const ChangeSettingSubject: React.FC<Props> = () => {
     ...
     * like TS __typename
     */
    const cleanupSubjects: (data: LFSubject[]) => LFSubject[] = useCallback((data: LFSubject[]) => {
        const arr: LFSubject[] = [];
    ...
    return (
        <>
            <WithNavigation headerTitle={t('profile.NeedHelpIn.single.header')} showBack isLoading={loading}>
                                ...
                                <Column marginRight={3} marginBottom={3} key={`selection-${index}`}>
                                    <Row alignItems="center" justifyContent="center">
                                        <TouchableOpacity
                                            onPress={() => {
                                                if (userType === 'student') {
                                                    setSelectedClassRange([subject?.grade?.min || 1, subject?.grade?.max || 13]);
                                                    setFocusedSelection({ name: subject.name });
                                                    setShowFocusSelection(true);
                                                } else {
                                                    if (!selections.find((s) => s.name === subject.name)) {
                                                        setSelections((prev) => [...prev, { name: subject.name }]);
                                                    }
                                                }
                                            }}
                                        >
                                                    ...
                                                    <IconTagList
                                                        ...
                                                        onPress={() => {
                                                            if (userType === 'student') {
                                                                ...
                                                                if (!selections.find((s) => s.name === subject.label)) {
                                                                    setSelections((prev) => [...prev, { name: subject.label }]);
                                                                }
                                                            }
                                                        }}
                                                    />
                                ...
                                {selections.find((sel) => sel.name === 'andere') && (
                                    <Row>
                                        <FormControl>
                                            <Stack>
                                                <FormControl.Label>
                                                    <Text bold>{t('profile.NeedHelpIn.single.optional.label')}</Text>
                                                </FormControl.Label>
                                                <Input
                                                    type="text"
                                                    multiline
                                                    numberOfLines={3}
                                                    h={70}
                                                    placeholder={t('profile.NeedHelpIn.single.optional.placeholder')}
                                                />
                                            </Stack>
                                        </FormControl>
                                    </Row>
                ...
                <VStack paddingX={space['1.5']} paddingBottom={space['1.5']} width="100%" marginX="auto" maxWidth={ContainerWidth}>
            ...
            <Modal isOpen={showFocusSelection} onClose={() => setShowFocusSelection(false)}>
                <Modal.Content>
                    <Modal.Header>
                        <Heading>
                    ...
                    <Modal.Body>
                        <Heading fontSize="md">
                            Klassen {selectedClassRange[0] || 1} - {selectedClassRange[1] || 13}
                        </Heading>
                        <Slider
                            animateTransitions
                            minimumValue={1}
                    ...
                    <Modal.Footer>
                        <Button
                            onPress={() => {
                                ...
                                const find = selections.findIndex((sel) => sel.name === focusedSelection?.name);
