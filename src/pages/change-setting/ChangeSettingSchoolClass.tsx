...
const ChangeSettingSchoolClass: React.FC<Props> = () => {
  ...
  const { data, loading } = useQuery(gql`query GetPupilSchool {
    me {
      pupil {
        schooltype
        gradeAsInt
  ...
  const [updateSchoolGrade, _updateSchoolGrade] = useMutation(gql`mutation updateSchoolGradePupil($grade: Int!) {
